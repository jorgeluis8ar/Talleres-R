...
geih3 <- function(path_file){
  ...
  if (indica=="Caracteristicas generales (Personas)"){
    ...
    base=base[,nombres]
    # names(base)[names(base) == "nivel"] <- paste0(archivo,"-",substr(indica,1,2)) # ayuda de "https://www.datanovia.com/en/lessons/rename-data-frame-columns-in-r/"
    names(base)[names(base) == "indicador"] <- substr(indica,1,3) # ayuda de "https://www.datanovia.com/en/lessons/rename-data-frame-columns-in-r/"
    names(base)[names(base) == "p6500"] <- "inglabo" # ayuda de "https://www.datanovia.com/en/lessons/rename-data-frame-columns-in-r/"
    names(base)[names(base) == "p6750"] <- "inglabodes" # ayuda de "https://www.datanovia.com/en/lessons/rename-data-frame-columns-in-r/"
  }
  return(base)
}
